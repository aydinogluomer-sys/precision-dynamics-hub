  ...
  return (
    <div className="space-y-4 animate-[fadeInUp_0.4s_ease-out]">
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-400">{customers.length} iş ortağı</span>
      ...
      {customers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {customers.map((c) => (
            <div key={c.id} className="bg-[#1E293B] rounded-xl border border-[#334155] p-5 hover:shadow-xl hover:border-[#0AA2CD]/30 transition-all group">
              ...
              <div className="space-y-2 text-xs">
                ...
                {c.balance !== null && (
                  <p className="text-white font-bold">Bakiye: ₺{Number(c.balance).toLocaleString("tr-TR")}</p>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-[#1E293B] rounded-xl border border-[#334155] w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-black text-white">Yeni İş Ortağı</h3>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setShowAdd(false)} className="flex-1 py-2 bg-slate-700 text-slate-300 rounded-lg text-xs font-bold">İptal</button>
