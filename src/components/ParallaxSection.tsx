...
export const ParallaxSection = forwardRef<HTMLDivElement, ParallaxSectionProps>(
  ({ children, className = "", index = 0, isLast = false, variant = "stack", style, videoBg }, _forwardedRef) => {
    ...
    const y = useTransform(
      scrollYProgress,
      [0, 1],
      variant === "slide-up" && !none ? [0, -120] : variant === "stack" && !none ? [0, -40] : [0, 0],
    );

    // --- Blur ---
    const blurValue = useTransform(scrollYProgress, [0, 1], variant === "zoom-out-blur" && !none ? [0, 10] : [0, 0]);

    // --- Wipe mask (diagonal polygon) ---
    const clipProgress = useTransform(scrollYProgress, [0, 1], variant === "wipe-mask" && !none ? [0, 100] : [0, 0]);
    ...
    // --- Depth-3d: rotateX ---
    const rotateXValue = useTransform(scrollYProgress, [0, 1], variant === "depth-3d" && !none ? [0, 6] : [0, 0]);
