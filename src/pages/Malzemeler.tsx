...
import { materialsData, materialCategoryDetails, type Material } from "@/data/materialsData";
...
const categoryFilters = [
  { id: "all", label: "Tümü" },
  { id: "metal", label: "Metaller" },
  { id: "plastic", label: "Plastikler" },
  { id: "composite", label: "Kompozitler" },
];

const subcategoryFilters: Record<string, { id: string; label: string }[]> = {
  metal: [
    { id: "all", label: "Tümü" },
    { id: "aluminum", label: "Alüminyum" },
    { id: "steel", label: "Çelik" },
    { id: "stainless", label: "Paslanmaz" },
    { id: "titanium", label: "Titanyum" },
    { id: "nickel", label: "Nikel" },
    { id: "brass", label: "Pirinç & Bronz" },
    { id: "copper", label: "Bakır" },
    { id: "magnesium", label: "Magnezyum" },
  ],
  plastic: [
    { id: "all", label: "Tümü" },
    { id: "thermoplastic", label: "Termoplastikler" },
    { id: "high-performance", label: "Yüksek Performans" },
  ],
  composite: [
    { id: "all", label: "Tümü" },
    { id: "composite", label: "Kompozit" },
  ],
};
...
const Malzemeler = () => {
  ...
  const filteredMaterials = useMemo(() => {
    ...
    if (subcategory !== "all") {
      result = result.filter((m) => m.subcategory === subcategory);
    }
    ...
    return result;
  }, [category, subcategory, sortBy, searchQuery]);

  useEffect(() => {
    setCurrentPage(1);
  }, [category, subcategory, sortBy, searchQuery, itemsPerPage]);
  ...
  const activeInfo = useMemo(() => {
    if (subcategory !== "all" && materialCategoryDetails[subcategory]) return materialCategoryDetails[subcategory];
    if (category !== "all" && materialCategoryDetails[category]) return materialCategoryDetails[category];
    return null;
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container-industrial relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-lg text-white/70 max-w-2xl mx-auto mb-8">
              {materialsData.length}+ malzeme ve alaşım ile endüstriyel ihtiyaçlarınıza çözüm. Karşılaştırın, seçin, üretin.
            </p>
            ...
            <div className="max-w-xl mx-auto relative">
              ...
              <input
                type="text"
                placeholder="Malzeme ara... (ör: 7075, PEEK, titanyum)"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 pr-4 py-3.5 rounded-lg bg-white/10 border border-white/20 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent backdrop-blur-sm"
              />
              {searchQuery && (
        ...
        {activeInfo && (
          ...
          >
            <div className="container-industrial py-8">
              <h2 className="text-xl font-bold text-foreground mb-2">{activeInfo.title}</h2>
              <p className="text-muted-foreground mb-4">{activeInfo.description}</p>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-sm font-semibold text-primary mb-2">Avantajlar</h3>
                  <ul className="space-y-1">
                    {activeInfo.advantages.map((a, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-muted-foreground">
                        <Check className="w-3.5 h-3.5 text-primary shrink-0" /> {a}
                      </li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-primary mb-2">Kullanım Alanları</h3>
                  <div className="flex flex-wrap gap-2">
                    {activeInfo.applications.map((a, i) => (
                      <span key={i} className="px-3 py-1 text-xs font-medium bg-muted text-muted-foreground rounded-full">{a}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="container-industrial py-4">
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Kategori</label>
              <div className="flex gap-1">
                {categoryFilters.map((f) => (
                  <button
                    key={f.id}
                    onClick={() => { setCategory(f.id); setSubcategory("all"); }}
            ...
            {/* Subcategory */}
            {category !== "all" && subcategoryFilters[category] && (
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Alt Kategori</label>
                <div className="flex gap-1 flex-wrap">
                  {subcategoryFilters[category].map((f) => (
                    <button
                      key={f.id}
                      onClick={() => setSubcategory(f.id)}
                      className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${
                        subcategory === f.id
                          ? "bg-primary text-primary-foreground"
                          : "bg-muted text-muted-foreground hover:bg-muted/80"
            ...
            <div className="flex items-end gap-3 ml-auto">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                  <ArrowUpDown className="w-3 h-3" /> Sırala
                </label>
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                  className="px-3 py-1.5 text-xs border border-border rounded bg-background text-foreground min-w-[180px]"
                >
                  {sortOptions.map((s) => (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className={`p-1.5 rounded border ${viewMode === "grid-3" ? "border-primary text-primary bg-primary/5" : "border-border text-muted-foreground"}`}
                  >
                    <LayoutGrid className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setViewMode("grid-2")}
                    className={`p-1.5 rounded border ${viewMode === "grid-2" ? "border-primary text-primary bg-primary/5" : "border-border text-muted-foreground"}`}
                  >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="px-3 py-1.5 text-xs border border-border rounded bg-background text-foreground"
                >
          ...
          <AnimatePresence>
            {compareList.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg flex items-center justify-between gap-4 flex-wrap"
              >
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-semibold text-foreground">Karşılaştır ({compareList.length}/4):</span>
                  {compareList.map((m) => (
                    <span key={m.id} className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-primary text-primary-foreground rounded-full">
                      {m.name}
                      <button onClick={() => toggleCompare(m)} className="hover:opacity-70">×</button>
                    </span>
                  ))}
                </div>
      ...
      <div className="container-industrial pt-6 pb-2">
        <p className="text-sm text-muted-foreground">
          <span className="font-bold text-foreground">{filteredMaterials.length}</span> malzeme bulundu
          ...
          {visibleMaterials.map((material) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-card border rounded-lg p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded ${price.cls}`}>
                    {price.text}
                  </span>
                </div>
                ...
                <h3 className="text-lg font-bold text-foreground mb-1.5 group-hover:text-primary transition-colors">
                  {material.name}
                  {material.isPopular && <span className="ml-2 text-xs text-amber-500">⭐</span>}
                </h3>
                ...
                <div className="grid grid-cols-2 gap-2 p-3 bg-muted/50 rounded-md mb-4">
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Yoğunluk</span>
                    <span className="text-sm font-semibold text-foreground">{material.density} g/cm³</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Mukavemet</span>
                    <span className="text-sm font-semibold text-foreground">{material.tensileStrength} MPa</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Sertlik</span>
                    <span className="text-sm font-semibold text-foreground">{material.hardness}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Maks. Sıcaklık</span>
                    <span className="text-sm font-semibold text-foreground">{material.maxTemperature}°C</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {material.applications.slice(0, 3).map((app, i) => (
                    <span key={i} className="px-2 py-0.5 text-[10px] font-medium bg-muted text-muted-foreground rounded">
                      {app}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-border">
                  <button
                    ...
                    className="flex-1 py-2 text-xs font-semibold bg-muted text-foreground rounded hover:bg-muted/80 transition-colors"
                  >
                    Detay
                  </button>
                  <button
                    onClick={() => toggleCompare(material)}
                    ...
                    className={`flex-1 py-2 text-xs font-semibold rounded border transition-all ${
                      isComparing
                        ...
                        : "border-primary text-primary hover:bg-primary/5 disabled:opacity-50 disabled:cursor-not-allowed"
                    }`}
                  >
                    {isComparing ? "✓ Seçildi" : "+ Karşılaştır"}
                  </button>
                </div>
              </motion.div>
            );
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-10">
            <button
              ...
              className="p-2 rounded border border-border text-muted-foreground hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-2 mb-2">
                  <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase rounded ${getPriceLabel(selectedMaterial.priceCategory).cls}`}>
                    {getPriceLabel(selectedMaterial.priceCategory).text}
                  </span>
              ...
              <div className="space-y-6 mt-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <SlidersHorizontal className="w-4 h-4 text-primary" /> Teknik Özellikler
                  </h4>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-sm font-bold text-foreground mb-2">✅ Avantajlar</h4>
                    <ul className="space-y-1.5">
                      {selectedMaterial.advantages.map((a, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                          <Check className="w-3.5 h-3.5 text-emerald-500 mt-0.5 shrink-0" /> {a}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-foreground mb-2">⚠️ Dikkat Edilmesi Gerekenler</h4>
                    <ul className="space-y-1.5">
                      {selectedMaterial.limitations.map((l, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                ...
                {/* Applications */}
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-2">Uygulama Alanları</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedMaterial.applications.map((app, i) => (
                      <span key={i} className="px-3 py-1 text-xs font-medium bg-primary/10 text-primary rounded-full">{app}</span>
                    ))}
                  </div>
                </div>

                <Link to="/iletisim" className="btn-industrial-primary w-full text-center block py-3 text-sm">
                  Bu Malzeme ile Teklif Al <ArrowRight className="w-4 h-4 inline ml-1" />
                </Link>
              </div>
            </>
          )}
      ...
      <Dialog open={showCompareModal && compareList.length > 0} onOpenChange={() => setShowCompareModal(false)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Malzeme Karşılaştırması</DialogTitle>
          </DialogHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {compareList.map((m) => (
                    <TableHead key={m.id} className="font-bold text-center">{m.name}</TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow>
                  <TableCell className="font-medium">Fiyat Kategorisi</TableCell>
                  {compareList.map((m) => (
                    <TableCell key={m.id} className="text-center">
                      <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded ${getPriceLabel(m.priceCategory).cls}`}>
                        {getPriceLabel(m.priceCategory).text}
                      </span>
                    </TableCell>
                  ))}
                </TableRow>
                {[
                  ...
                  <TableRow key={row.key}>
                    ...
                    {compareList.map((m) => (
                      <TableCell key={m.id} className="text-center font-semibold">
                        {String((m as any)[row.key])}{row.suffix || ""}
                      </TableCell>
                    ))}
                  </TableRow>
                ))}
      ...
      <section className="py-16" style={{ background: "linear-gradient(135deg, hsl(var(--primary)) 0%, #0f172a 100%)" }}>
        <div className="container-industrial text-center">
          ...
          <p className="text-white/70 mb-8 max-w-lg mx-auto">
            Mühendislik ekibimiz projeniz için en uygun malzemeyi seçmenizde size yardımcı olabilir.
          </p>
          <Link to="/iletisim" className="btn-industrial-primary inline-block px-8 py-3">
